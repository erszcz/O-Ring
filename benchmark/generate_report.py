...
def generate_svg(data, nodes, iterations, title, y_axis_label, data_key):
    ...
    if not data:
        ...
        for lang_data in data.values():
            max_value = max(
                max_value,
                lang_data[data_key]["median"],
                lang_data[data_key]["p98"],
                lang_data[data_key]["mean"],
                lang_data[data_key]["rms"],
            )
...
<g transform="translate({margin["left"]}, {margin["top"]})">"""
    ...
    for i, (label, values) in enumerate(sorted_data):
        ...
        # RMS bar
        rms_height = (
            (values[data_key]["rms"] / max_value) * chart_height if max_value > 0 else 0
        )
        svg += f'<rect x="{x + bar_width * 0.5}" y="{chart_height - rms_height}" width="{bar_width}" height="{rms_height}" fill="#e15759" />\n'
        # Mean bar
        mean_height = (
            (values[data_key]["mean"] / max_value) * chart_height
            if max_value > 0
            else 0
        )
        ...
        # Median bar
        median_height = (
            (values[data_key]["median"] / max_value) * chart_height
            if max_value > 0
            else 0
        )
        ...
        # 98th percentile bar
        p98_height = (
            (values[data_key]["p98"] / max_value) * chart_height if max_value > 0 else 0
        )
...
def main(args):
    ...
    if plot_data:
        setup_svg = generate_svg(
            plot_data,
            nodes,
            iterations,
            "O-Ring Benchmark Setup Times",
            "Setup Time",
            "setup",
        )
        runtime_svg = generate_svg(
            plot_data,
            nodes,
            iterations,
            "O-Ring Benchmark Runtime Results",
            "Time to Finish",
            "runtime",
        )
